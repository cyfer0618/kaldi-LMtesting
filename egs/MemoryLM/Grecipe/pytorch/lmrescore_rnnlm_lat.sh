...
if [ $# != 5 ]; then
   ...
   echo " echo e.g.: $0 data/lang_test_tg data/pytorch data/test exp/chain/tdnn1g_sp/decode_tg_test exp/chain/test_pyrnnlm"
...
[ ! -d $rnnlm_dir/rnnlm ] && echo "$0: Missing pytorch model folder $rnnlm_dir/rnnlm" && exit 1;
...
if [ "$oldlm" == "$oldlang/G.fst" ]; then
  ...
  #   "ark:gunzip -c $indir/lat.JOB.gz|" "$oldlm_command" ark:-  \| LMtesting \| \
  #   lattice-lmrescore-py-rnnlm --lm-scale=$weight \
